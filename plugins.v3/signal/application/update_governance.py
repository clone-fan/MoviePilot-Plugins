...
class UpdateGovernanceMixin:
    ...
    def _dispatch_moviepilot_upgrade(data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            upgrade_dev = getattr(SystemHelper, "upgrade_dev", None)
            if not callable(upgrade_dev):
                mp["upgrade_error"] = "当前 MoviePilot 未提供插件可用的升级入口"
                return
            ok, message = upgrade_dev()
            mp["upgrade_dispatched"] = bool(ok)
            mp["upgrade_channel"] = "dev"
            if message:
    ...
    @staticmethod
    def _host_major_version() -> int:
        """读取宿主主版本，用于选择同代 release 通道。

        Signal 3.0.0 只对 MoviePilot V3 负责，因此默认代际为 3；只有宿主明确自报
        其它主版本时才跟随，避免把 V2 的最新版拿来和 V3 本地版本比较。
        """
        try:
            from version import APP_VERSION

            nums = re.findall(r"\d+", str(APP_VERSION or ""))
            if nums:
                return int(nums[0])
        except Exception:
            pass
