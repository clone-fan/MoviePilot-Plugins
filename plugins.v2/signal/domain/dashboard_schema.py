...
class DashboardSchemaMixin:
    ...
    def _task_definitions(self) -> List[Dict[str, Any]]:
        return [
            ...
            {"key": "backup", "name": "自动备份", "enabled": self._backup_enabled, "schedule_required": True, "schedule_enabled": self._backup_enabled, "cron": self._backup_cron, "last_keys": ["last_backup"], "next": self._backup_cron, "icon": "mdi-database-arrow-up"},
            ...
            {"key": "market_update", "name": "插件库同步", "enabled": self._market_update_enabled, "schedule_required": True, "schedule_enabled": self._market_update_schedule_enabled, "cron": self._market_update_cron, "last_keys": ["last_market_update"], "next": self._market_update_cron, "icon": "mdi-puzzle-check"},
            {"key": "plugin_update_reminder", "name": "插件更新", "enabled": self._plugin_update_reminder_enabled, "schedule_required": True, "schedule_enabled": self._plugin_update_reminder_schedule_enabled, "cron": self._plugin_update_reminder_cron, "last_keys": ["last_plugin_update_reminder"], "next": self._plugin_update_reminder_cron, "icon": "mdi-bell-alert-outline"},
    ...
    def _action_button(text: str, icon: str, color: str, api_path: str) -> dict:
        return {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VBtn", "props": {"block": True, "variant": "tonal", "color": color, "prepend-icon": icon, "class": "text-none mb-2"}, "text": text, "events": {"click": {"api": f"plugin/Signal/{api_path}", "method": "post"}}}]}
