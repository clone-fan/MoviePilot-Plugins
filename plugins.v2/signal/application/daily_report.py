...
class DailyReportMixin:
    ...
    def _build_summary(self) -> str:
        return "；".join([f"插件：{'启用' if self._enabled else '未启用'}", f"每日汇报：{'启用' if self._daily_report_enabled else '停用'} {self._daily_report_cron}", f"汇报栏目：健康={'开' if self._health_in_report else '关'} / 订阅={'开' if self._subscribe_in_report else '关'} / 站点={'开' if self._site_stat_in_report else '关'}", f"插件日志清理：{'启用' if self._log_clean_enabled else '停用'} {self._log_clean_cron} 保留{self._log_clean_rows}行", f"自动备份：{'启用' if self._backup_enabled else '停用'} {self._backup_cron} 保留{self._backup_keep_count}个", f"更新检查：{'启用' if self._mp_update_enabled else '停用'} {self._mp_update_cron}", f"插件库检查：{'启用' if self._market_update_enabled else '停用'} {self._market_update_cron}"])
